...
- `catalogs/spl/commands.toml` — **118** commands, **144** functions (**109** eval + **35** aggregate)
...
## A. Commands — in Search Reference, **not** in catalog / grammar

These parse as `unknown_command` today (engine can emit `spl_unknown_command`):
