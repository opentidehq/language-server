...
| --- | --- | --- | --- |
| Commands (`[[commands]]`) | **118** | **~115** unique primary names in [commands.md](commands.md) (+ aliases / adjacent) | **~32** Search Reference commands still absent (mostly rare / UI / ML / admin). Detection-relevant set is covered. |
...
**Catalog ↔ grammar:** 118 commands = 17 dedicated parse rules + 101 `catalog_command_name` literals. Sets match (no catalog↔grammar drift).

**Completions:** after `|` → all 118 commands; otherwise → all catalog functions.
